...
- "At the start of this project I had zero FedRAMP experience — I've built AI inside classified networks, but
  never walked in the front door. This is the tool I built to learn it — and it runs on
  FedRAMP's own published data."
- Point at the hero tile: "Median about 67 days to certified on the new 20x path,
  versus about 328 on the legacy path. I couldn't find that number published
...
**3:30–4:20 — Security and verification**
...
- Name the layers while showing the about-page security note: "Downloads are bounded and pinned
  to an immutable Git blob; strings are sanitized at the shared transform boundary, escaped at
  render sinks, and constrained by CSP. Hostile fixtures exercise the same path in CI."
- "Read-only reviews found real defects in feed rendering, refresh consistency, journey
  ordering, and statistics. Each accepted finding got a focused regression test before the
  published snapshots and claims were regenerated."
